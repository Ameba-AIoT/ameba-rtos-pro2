...
static mm_siso_t *lr_siso_aac_mp4 = NULL;
...
int lifetime_audio_initialize(uint8_t record_filename_length, const char *filename)
{
	...
	lr_siso_aac_mp4 = siso_create();
    if (lr_siso_aac_mp4) {
        siso_ctrl(lr_siso_aac_mp4, MMIC_CMD_ADD_INPUT, (uint32_t)lr_aac_ctx, 0);
        siso_ctrl(lr_siso_aac_mp4, MMIC_CMD_ADD_OUTPUT, (uint32_t)lr_mp4_ctx, 0);
        siso_ctrl(lr_siso_aac_mp4, MMIC_CMD_SET_TASKPRIORITY, 4, 0);
        siso_start(lr_siso_aac_mp4);
    } else {
...
void lifetime_audio_deinitialize(void)
{
	...
	siso_delete(lr_siso_aac_mp4);
    lr_siso_aac_mp4 = NULL;
